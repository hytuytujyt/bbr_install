...
> ⚠️ 上面的命令最后带了 `DISABLE_PASSWORD=0`（即**跳过关闭密码登录**）。若你想**默认关闭密码登录**，把这一行里的 `DISABLE_PASSWORD=0 `（含后面的空格）删掉再执行即可。
...
> ```
>
> 办法二（还没下载、用上面的"一键安装"整行）：把那一行末尾的 `DISABLE_PASSWORD=0 `（含后面的空格）**删掉**再执行，就会默认关闭密码登录。

**不想关闭密码登录？** 两种方式：本地执行时前面加 `DISABLE_PASSWORD=0 `（注意后面要有空格）：
>
> ```
> DISABLE_PASSWORD=0 sh install_bbr.sh
> ```
> 或用上面"一键安装"整行（它尾部已自带 `DISABLE_PASSWORD=0`，直接执行即可跳过关闭密码登录）。
