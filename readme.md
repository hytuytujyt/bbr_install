...
关闭逻辑说明：
- 脚本维护自己的 `/etc/ssh/sshd_config.d/00-disable-password.conf`，不会删除或重排服务商的 `99-runman.conf` 等文件。
- 为处理主配置提前写入 `PasswordAuthentication yes` 的情况，脚本只在 `/etc/ssh/sshd_config` 最前面加入自己的 `Include /etc/ssh/sshd_config.d/00-disable-password.conf`；系统原有的 `Include /etc/ssh/sshd_config.d/*.conf` 会保留原位置和顺序。
- 修改前会备份主配置和原有的 `00-disable-password.conf` 到 `/etc/ssh/bbr-install-backup-时间戳/`。写入失败或校验失败时恢复这两份文件。
- 会先检查原有配置，再运行 `sshd -t` 和 `sshd -T`；语法错误、有效值不是 `passwordauthentication no`、交互认证未确认为关闭、SSH 重启失败或重启后校验失败，都会恢复备份并返回失败，不会误报成功。
- 脚本会根据当前 OpenSSH 支持情况写入 `KbdInteractiveAuthentication no`、`ChallengeResponseAuthentication no` 中可用的配置项。新版 OpenSSH 通常只输出前者，后者不输出属于正常版本差异；脚本不会为了本次操作自动注释服务商的其他未知配置项。
- 脚本会重启 SSH 服务，但**不需要重启整个系统**。已有 SSH 会话通常不会被主动断开，但仍应保留当前窗口，并在另一窗口测试密钥登录。如果服务重启或恢复重启失败，请根据脚本提示手动检查 SSH 服务状态。
...
关闭密码登录后，若想查看当前密码认证是开是关，可随时执行：

```sh
sshd -T -f /etc/ssh/sshd_config | grep -iE '^(passwordauthentication|kbdinteractiveauthentication|challengeresponseauthentication)[[:space:]]'
```

判定标准：
- `passwordauthentication no` → sshd 解析出的密码认证已关闭 ✅
- `passwordauthentication yes` → 密码认证仍开启，不能视为成功
- `kbdinteractiveauthentication no` → 新版 OpenSSH 的键盘交互认证已关闭
- 如果没有 `kbdinteractiveauthentication`，但有 `challengeresponseauthentication no` → 旧版 OpenSSH 的等效设置已关闭
- 新版 OpenSSH 不输出 `challengeresponseauthentication` 很正常；但如果两个设置都没有输出，就不能据此确认键盘交互认证已关闭

这些命令只能显示 sshd 的配置解析结果，**不能证明公钥登录一定可用**。脚本运行结束后，请用另一窗口验证：密码登录已失效、密钥登录正常；BBR 状态用 `bbrstatus` 查看。

如果你的机器有服务商生成的 SSH 配置（例如 `99-runman.conf`），不要直接删除或重排它；先检查加载顺序：
