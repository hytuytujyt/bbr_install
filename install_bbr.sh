...
disable_password_login() {
  ...
  local guard_include_line="Include $drop_file"
  ...
  local tmp_conf tmp_drop check_err check_out pa kbd cr
  local kbd_supported=0 cr_supported=0
  local ssh_restart_attempted=0
  ...
  rollback_ssh() {
    ...
    _err2 "正在恢复 SSH 配置 ..."
    if ! cp -p "$main_backup" "$conf_file"; then
      _err2 "恢复 $conf_file 失败"
      rollback_status=1
    fi
    if [ "$drop_existed" = "1" ]; then
      if ! cp -p "$drop_backup" "$drop_file"; then
        _err2 "恢复 $drop_file 失败"
        rollback_status=1
      fi
    elif ! rm -f "$drop_file"; then
      _err2 "删除新建的 $drop_file 失败"
      rollback_status=1
    fi
    if ! sshd -t -f "$conf_file" >/dev/null 2>&1; then
      _err2 "恢复后的 SSH 配置仍未通过语法检查"
      rollback_status=1
    fi
    if [ "$ssh_restart_attempted" = "1" ]; then
      if [ "$rollback_status" = "0" ] && restart_ssh_service; then
        _info2 "已重启 SSH 使恢复的旧配置生效"
      else
        _err2 "恢复文件已写回,但无法确认旧配置已重新加载;请手动检查并重启 SSH 服务"
        rollback_status=1
      fi
    fi
  ...
  # 兼容不同 OpenSSH 版本：新版本使用 KbdInteractiveAuthentication，
  # 旧版本可能只接受 ChallengeResponseAuthentication。
  if sshd -T -f "$conf_file" -o KbdInteractiveAuthentication=no >/dev/null 2>&1; then
    kbd_supported=1
  fi
  if sshd -T -f "$conf_file" -o ChallengeResponseAuthentication=no >/dev/null 2>&1; then
    cr_supported=1
  fi
  if [ "$kbd_supported" = "0" ] && [ "$cr_supported" = "0" ]; then
    _err2 "当前 sshd 不支持可用的键盘交互认证配置项, 未修改 SSH"
    rm -rf "$backup_dir"
    return 1
  fi

  tmp_drop="$(mktemp /tmp/sshd_drop.XXXXXX)" || {
    _err2 "无法创建 SSH drop-in 临时文件"
    rm -rf "$backup_dir"
    return 1
  }
  {
    printf '%s\n' "PasswordAuthentication no"
    if [ "$kbd_supported" = "1" ]; then
      printf '%s\n' "KbdInteractiveAuthentication no"
    fi
    if [ "$cr_supported" = "1" ]; then
  ...
  if ! install -m 600 "$tmp_drop" "$drop_file"; then
  ...
  # 只把脚本自己的单文件 Include 放到最前面；保留系统/供应商原有
  # 的 *.conf Include 位置，避免改变其他 SSH 配置的作用域和顺序。
  ...
  {
    printf '%s\n' "$guard_include_line"
    awk -v guard="$guard_include_line" '
      function trim(s) {
        sub(/^[[:space:]]+/, "", s)
        sub(/[[:space:]]+$/, "", s)
        return s
      ...
      {
        line=trim($0)
        if (line == guard || (index(line, guard) == 1 && substr(line, length(guard) + 1) ~ /^[[:space:]]+#/)) next
        print
      }
    ' "$conf_file"
  } > "$tmp_conf" || {
    _err2 "无法生成新的 SSH 主配置"
    rm -f "$tmp_conf"
    rollback_ssh
    return 1
  }
  if ! install -m 600 "$tmp_conf" "$conf_file"; then
    _err2 "无法写入 $conf_file"
    rm -f "$tmp_conf"
    rollback_ssh
    return 1
  fi
  ...
  if [ "$pa" != "no" ]; then
    _err2 "校验失败: 密码认证可能仍为开启(pa=$pa kbd=$kbd cr=$cr), 未重启 SSH"
    _err2 "请手动复核: sshd -T -f '$conf_file' | grep -iE 'password|kbdinteractive|challengeresponse'"
    rollback_ssh
    return 1
  fi
  if { [ -n "$kbd" ] && [ "$kbd" != "no" ]; } || { [ -n "$cr" ] && [ "$cr" != "no" ]; } || { [ -z "$kbd" ] && [ -z "$cr" ]; }; then
    _err2 "校验失败: 键盘交互认证未确认为关闭(kbd=$kbd cr=$cr), 未重启 SSH"
    rollback_ssh
    return 1
  fi

  _info2 "正在重启 SSH 服务使配置生效 ..."
  ssh_restart_attempted=1
  ...
  if [ "$pa" != "no" ] || { [ -n "$kbd" ] && [ "$kbd" != "no" ]; } || { [ -n "$cr" ] && [ "$cr" != "no" ]; } || { [ -z "$kbd" ] && [ -z "$cr" ]; }; then
    _err2 "重启后校验失败: 密码认证或键盘交互认证仍未关闭(pa=$pa kbd=$kbd cr=$cr)"
    _err2 "正在恢复修改前的 SSH 配置；现有 SSH 会话不会被主动断开"
    rollback_ssh
    return 1
  fi

  _ok2 "校验通过: 密码认证已确认为关闭。"
