...
disable_password_login() {
  ...
  # 当前 Alpine/OpenSSH 可能不支持 UsePAM。只在 sshd 明确报 Unsupported
  # option UsePAM 时注释该无效项；原文件已保存在 backup_dir 中。
  ...
  if sshd -t -f "$conf_file" >/dev/null 2>"$check_err"; then
    :
  elif grep -qiE 'unsupported option[[:space:]]+UsePAM' "$check_err"; then
    _warn2 "当前 sshd 不支持 UsePAM, 已临时注释该无效配置(原文件已备份)"
    tmp_conf="$(mktemp /tmp/sshd_config.XXXXXX)" || {
      rm -f "$check_err"
      rollback_ssh
      return 1
    }
    awk '/^[[:space:]]*[Uu][Ss][Ee][Pp][Aa][Mm]([[:space:]]|$)/ { print "# install_bbr.sh: 当前 sshd 不支持 UsePAM"; print "# " $0; next } { print }' "$conf_file" > "$tmp_conf" || {
